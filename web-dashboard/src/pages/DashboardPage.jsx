...
export default function DashboardPage() {
  ...
  // States for GAP 1 & GAP 2: Filters and Drill-down
  const [drilledCentre, setDrilledCentre] = useState(null)
  const [filters, setFilters] = useState({ period: 'this_month', ageGroup: 'all', sex: 'all', status: 'all' })
  ...
  // DRILL DOWN VIEW FOR SECTOR COORDINATOR
  if (drilledCentre) {
    return (
      <div className="flex-1 overflow-auto min-h-0 bg-stone-50">
        ...
        <div className="p-6 space-y-6">
          <button 
            ...
            className="flex items-center gap-2 text-sm font-semibold text-[#0f2d26] hover:underline"
          >
          ...
          <div className="bg-white rounded-xl border border-stone-200/90 shadow-sm p-8 text-center">
            ...
            <button className="mt-4 px-4 py-2 bg-[#0f2d26] text-white rounded font-semibold shadow hover:bg-[#163d34]">
              Request Full Centre Audit Report
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 overflow-auto min-h-0">
      ...
      <div className="p-6 space-y-6">
        ...
        {/* GAP 2: DASHBOARD FILTERS (For District, National, Partner) */}
        {['district', 'national', 'sys_admin', 'partner'].includes(role) && (
          <div className="bg-white rounded-xl border border-stone-200/90 shadow-sm p-4 flex flex-wrap gap-4 items-end">
            ...
            <button className="px-4 py-1.5 bg-[#0f2d26] text-white text-sm font-semibold rounded-md hover:bg-[#163d34] shadow-sm">
              Apply Filters
            </button>
          </div>
        )}
        ...
        {!user ? (
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-red-900">
            ...
            <button type="button" onClick={() => refetch()} className="mt-4 px-4 py-2 bg-[#0f2d26] text-white text-sm rounded-lg hover:bg-[#163d34]">
              Retry
            </button>
          </div>
        ) : (
          <>
            ...
            {/* GAP 1: DRILL-DOWN LINKS FOR CENTRES TABLE */}
            {d.centres && (
              <div className="bg-white rounded-xl border border-stone-200/90 shadow-sm p-5">
                ...
                <p className="text-xs text-stone-500 -mt-2 mb-4">Sorted by urgency. Click a row to view details.</p>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {d.centres.map((c, i) => (
                        <tr 
                          key={c.centre_id} 
                          onClick={() => setDrilledCentre(c)}
                          className={`${i % 2 === 0 ? 'bg-white' : 'bg-stone-50/80'} hover:bg-teal-50 cursor-pointer transition-colors`}
                        >
                          ...
                          <td className="px-4 py-2.5 text-center tabular-nums">
                            <span className={`font-semibold ${c.sam_percent > 10 ? 'text-red-700' : c.sam_percent > 5 ? 'text-amber-700' : 'text-emerald-700'}`}>
                              {c.sam_percent}%
                            </span>
                          </td>
                          ...
                          <td className="px-4 py-2.5 text-right">
                            <button className="text-xs font-bold text-teal-700 hover:underline">View &rarr;</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {d.due_or_overdue?.length > 0 && (
              <div className="bg-white rounded-xl border border-stone-200/90 shadow-sm p-5">
                <h3 className="font-display font-semibold text-stone-900 mb-1">Due or overdue for measurement</h3>
                <p className="text-xs text-stone-500 mb-4">{d.due_or_overdue.length} children</p>
                <div className="space-y-2">
                  {d.due_or_overdue.slice(0, 8).map(c => (
                    <div key={c.child_id} className="flex items-center justify-between gap-3 p-3 rounded-lg border border-amber-200/80 bg-amber-50/50">
                      <span className="font-medium text-stone-800">{c.name}</span>
                      <span className="text-xs text-amber-900/80 font-medium shrink-0">
                        {c.days_since != null ? `${c.days_since} days since last measurement` : 'Never measured'}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            {['district', 'national', 'sys_admin', 'partner'].includes(role) && d.centres && (
              <div className="bg-white rounded-xl border border-stone-200/90 shadow-sm p-5 mt-6">
                ...
                <p className="text-xs text-stone-500 mb-4">Red circles indicate high SAM/MAM concentrations based on current filters.</p>
                  ...
                  <MapContainer center={[-1.9403, 29.8739]} zoom={8} scrollWheelZoom={false} className="h-full w-full">
                    ...
                    {d.centres.map(c => (
                       <CircleMarker
                         ...
                         pathOptions={{ fillColor: c.sam_percent > 5 ? '#ef4444' : '#22c55e', color: 'white', weight: 1, fillOpacity: 0.7 }}
                       >
                         <Popup>
